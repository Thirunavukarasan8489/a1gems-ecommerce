...
export function RashiFinder() {
  const [selected, setSelected] = React.useState(rashiList[4].slug); // Simha/Leo
  const rashi = rashiList.find((r) => r.slug === selected) ?? rashiList[4];
  const category = rashi.categorySlug ? getCategory(rashi.categorySlug) : undefined;
      ...
      <div className="shell gutter">
        ...
        <div className="text-center">
          <p className="text-[0.6875rem] font-semibold tracking-[0.18em] text-gold-700 uppercase">
            Rashi Ratna ·{" "}
            <span className="font-devanagari tracking-normal normal-case">
              राशि रत्न
            </span>
          </p>
          ...
          <div className="flex items-center gap-4 bg-plum-950 p-5 text-ivory-100">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-1.5 text-sm text-plum-300">
                {rashi.transliteration} · {rashi.english} · {rashi.dateRange}
              </p>
            </div>
          </div>
          ...
          <div className="p-5 sm:p-6">
            ...
            {category ? (
              ...
              <p className="mt-5 rounded-xl bg-ivory-200 px-4 py-3 text-[0.8125rem] leading-relaxed text-plum-800">
                ...
                <Link href="/contact" className="font-semibold text-gold-700 underline underline-offset-2">
                  Ask us to source it
                </Link>{" "}
                for you.
              </p>
            ...
            <p className="mt-4 flex items-start gap-2 text-xs leading-relaxed text-ink-muted">
              ...
              A general reference, not personal advice — birth chart, ascendant
              and current dashas can change what is right for you. Please
              consult your astrologer before purchase.
            </p>
          </div>
        </div>
      </div>
