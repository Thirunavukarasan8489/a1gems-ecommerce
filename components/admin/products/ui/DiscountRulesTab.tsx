  ...
  return (
    <div className={isActive ? 'space-y-5' : 'hidden'}>
      <div className="flex items-center justify-between border-b border-gold-100 dark:border-gold-800 pb-3">
        <div>
          <h2 className="text-lg font-semibold text-gold-900 dark:text-white">Bulk Discount Rules</h2>
          <p className="text-xs text-gold-500 mt-1">Add quantity-based discounts. If the category evaluates based on Variant Value, &apos;Qty&apos; refers to the total value (e.g. Carats) instead of pieces.</p>
        </div>
      ...
      <div className="space-y-3">
        {ruleFields.length === 0 && (
          <div className="p-4 border border-gold-200 dark:border-gold-800 border-dashed rounded-xl text-center text-gold-500 text-sm">
            No discount rules applied. Click &quot;Add Rule&quot; to set up tier pricing.
          </div>
        )}
        {ruleFields.map((field, index) => (
          <div key={field.id} className="flex items-start gap-4 p-4 border border-gold-200 dark:border-gold-700 rounded-xl bg-white dark:bg-gold-900 relative">
